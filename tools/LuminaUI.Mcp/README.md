...
Docs MCP Docker 发布不会创建 GitHub Release，也不会创建 Git tag。GitHub Release 只保留给 NuGet 包发布流程，避免 docs MCP Docker 和 diagnostics MCP 在 Releases/Tags 页面混在一起。重复版本由 Docker Hub 镜像 tag 校验：如果 `${LuminaDocsMcpVersion}` tag 已存在，workflow 会跳过 build/push。
