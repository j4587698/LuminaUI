...
public sealed class LuminaImageLoader : ILuminaImageLoader
{
	...
	public async Task<IImage?> LoadAsync(object? source, LuminaImageLoadOptions options, CancellationToken cancellationToken)
	{
		...
		string? text = ((source is Uri uri) ? uri.ToString() : ((!(source is string text2)) ? source.ToString() : text2));
		if (1 == 0)
		{
		}
	...
	private static IImage? LoadLocalImage(string source)
	{
		...
		{
			using (FileStream stream2 = File.OpenRead(source))
			{
				return Decode(stream2);
			}
		}
		if (Uri.TryCreate(source, UriKind.Absolute, out uri) && uri.IsFile && File.Exists(uri.LocalPath))
		{
			using (FileStream stream3 = File.OpenRead(uri.LocalPath))
			{
				return Decode(stream3);
			}
		}
		return null;
	}
	...
	private static bool TryDecodeBase64(string source, out byte[] bytes)
	{
		...
		if (source.StartsWith("data:", StringComparison.OrdinalIgnoreCase))
		{
			...
			int num = commaIndex + 1;
			payload = source.Substring(num, source.Length - num);
		}
	...
	{
		payload = string.Concat(payload.Where((char c) => !char.IsWhiteSpace(c))).Replace('-', '+').Replace('_', '/');
