...
public class LuminaRatingItem : TemplatedControl
{
	...
	public static readonly StyledProperty<object?> CharacterProperty = AvaloniaProperty.Register<LuminaRatingItem, object?>("Character", "★");

	public static readonly StyledProperty<bool> AllowHalfProperty = AvaloniaProperty.Register<LuminaRatingItem, bool>("AllowHalf", defaultValue: false);

	public static readonly StyledProperty<double> SizeProperty = AvaloniaProperty.Register<LuminaRatingItem, double>("Size", 24.0);

	public static readonly DirectProperty<LuminaRatingItem, double> SelectedWidthProperty = AvaloniaProperty.RegisterDirect<LuminaRatingItem, double>("SelectedWidth", (LuminaRatingItem item) => item.SelectedWidth, null, 0.0);
	...
	internal void SetSelectedRatio(double ratio)
	{
		...
		base.PseudoClasses.Set(":selected", _selectedRatio > 0.0);
		UpdateSelectedWidth();
	}
	...
	private void UpdateHalfPreview(PointerEventArgs e)
	{
		...
		{
			_isHalfPreview = e.GetPosition(this).X <= base.Bounds.Width * 0.5;
		}
	}

	private void UpdateSelectedWidth()
	{
		SelectedWidth = Math.Max(0.0, base.Bounds.Width * _selectedRatio);
