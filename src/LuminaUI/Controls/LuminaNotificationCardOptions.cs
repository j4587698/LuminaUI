...
public class LuminaNotificationCardOptions : AvaloniaObject
{
    ...
    {
        if (sender is not WindowNotificationManager manager)
        {
            return;
        }
        foreach (NotificationCard notificationCard in manager.GetVisualDescendants().OfType<NotificationCard>())
    ...
    {
        if (sender is not NotificationCard notificationCard || e.Property != NotificationCard.IsClosingProperty || !e.GetNewValue<bool>())
        {
            return;
        }
