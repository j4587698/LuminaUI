...
public sealed class LuminaButton
{
	...
	static LuminaButton()
	{
		...
		IconProperty.Changed.AddClassHandler(delegate(Button button, AvaloniaPropertyChangedEventArgs _)
		{
			UpdateState(button);
		});
		ContentControl.ContentProperty.Changed.AddClassHandler(delegate(Button button, AvaloniaPropertyChangedEventArgs _)
		{
			UpdateState(button);
		});
	}
