...
public class LuminaCarouselOptions : AvaloniaObject
{
	...
	static LuminaCarouselOptions()
	{
		...
		AutoPlayIntervalProperty = AvaloniaProperty.RegisterAttached<LuminaCarouselOptions, Carousel, TimeSpan>("AutoPlayInterval", TimeSpan.FromSeconds(3L));
		...
		IsAutoPlayEnabledProperty.Changed.AddClassHandler(delegate(Carousel carousel, AvaloniaPropertyChangedEventArgs _)
		{
			UpdateAutoPlay(carousel);
		});
		AutoPlayIntervalProperty.Changed.AddClassHandler(delegate(Carousel carousel, AvaloniaPropertyChangedEventArgs _)
		{
			UpdateAutoPlayInterval(carousel);
		});
		PauseOnPointerOverProperty.Changed.AddClassHandler(delegate(Carousel carousel, AvaloniaPropertyChangedEventArgs _)
		{
			UpdateAutoPlay(carousel);
		});
		ShowNavigationButtonsProperty.Changed.AddClassHandler(delegate(Carousel carousel, AvaloniaPropertyChangedEventArgs _)
		{
			UpdateVisibilityClasses(carousel);
		});
		ShowIndicatorsProperty.Changed.AddClassHandler(delegate(Carousel carousel, AvaloniaPropertyChangedEventArgs _)
		{
			UpdateVisibilityClasses(carousel);
		});
	}
	...
	{
		return (interval < TimeSpan.FromMilliseconds(500L)) ? TimeSpan.FromMilliseconds(500L) : interval;
	}
