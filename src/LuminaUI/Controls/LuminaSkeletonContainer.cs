...
public class LuminaSkeletonContainer : ContentControl
{
	...
	public static readonly StyledProperty<bool> IsLoadingProperty = AvaloniaProperty.Register<LuminaSkeletonContainer, bool>("IsLoading", defaultValue: false);

	public static readonly StyledProperty<LuminaSkeletonContainerMode> ModeProperty = AvaloniaProperty.Register<LuminaSkeletonContainer, LuminaSkeletonContainerMode>("Mode", LuminaSkeletonContainerMode.Auto);

	public static readonly StyledProperty<int> PlaceholderCountProperty = AvaloniaProperty.Register<LuminaSkeletonContainer, int>("PlaceholderCount", 4);

	public static readonly StyledProperty<object?> SkeletonContentProperty = AvaloniaProperty.Register<LuminaSkeletonContainer, object?>("SkeletonContent");

	public static readonly StyledProperty<IDataTemplate?> SkeletonContentTemplateProperty = AvaloniaProperty.Register<LuminaSkeletonContainer, IDataTemplate?>("SkeletonContentTemplate");
	...
	public static readonly DirectProperty<LuminaSkeletonContainer, bool> HasActiveSkeletonContentProperty = AvaloniaProperty.RegisterDirect<LuminaSkeletonContainer, bool>("HasActiveSkeletonContent", (LuminaSkeletonContainer container) => container.HasActiveSkeletonContent, null, unsetValue: false);
	...
	public int PlaceholderCount
	{
		get
		{
			return GetValue(PlaceholderCountProperty);
		}
		set
		{
			SetValue(PlaceholderCountProperty, value);
		}
	}
	...
	public object? SkeletonContent
	{
		get
		{
			return GetValue(SkeletonContentProperty);
		}
		set
		{
			SetValue(SkeletonContentProperty, value);
		}
	}
	...
	public IDataTemplate? SkeletonContentTemplate
	{
		get
		{
			return GetValue(SkeletonContentTemplateProperty);
		}
		set
		{
			SetValue(SkeletonContentTemplateProperty, value);
		}
	}
	...
	protected override void OnPropertyChanged(AvaloniaPropertyChangedEventArgs change)
	{
		...
		{
			base.PseudoClasses.Set(":loading", IsLoading);
			UpdateSkeletonContent();
			QueueAutoRefresh();
		}
	...
	private void UpdateSkeletonContent()
	{
		...
		if (IsLoading)
		{
			...
			{
				content = BuildAutoSkeleton(base.Content);
			}
		}
	...
	private void UpdateLoadingVisualState()
	{
		...
		{
			_contentPresenter.Opacity = ((!shouldShowSkeleton) ? 1 : 0);
	...
	private Control BuildAutoSkeleton(object? content)
	{
		...
		Control result = ((content is Control control) ? (BuildSkeletonForControl(control) ?? CreateSkeleton(null, LuminaSkeletonShape.Block, 240.0, 96.0)) : ((!(content is string text)) ? CreateSkeleton(null, LuminaSkeletonShape.Block, 240.0, 96.0) : CreateTextSkeleton(null, text, LuminaSkeletonShape.Text)));
		if (1 == 0)
		{
		}
		return result;
	...
	private Control? BuildSkeletonForControl(Control source)
	{
		...
		Control result = ((source is TextBlock textBlock) ? CreateTextSkeleton(textBlock, textBlock.Text, GuessTextShape(textBlock)) : ((source is Image) ? CreateSkeleton(source, GuessImageShape(source), 96.0, 96.0) : ((source is LuminaImage) ? CreateSkeleton(source, GuessImageShape(source), 96.0, 96.0) : ((source is LuminaAvatar) ? CreateSkeleton(source, LuminaSkeletonShape.Circle, 44.0, 44.0) : ((source is Button) ? CreateSkeleton(source, LuminaSkeletonShape.Rectangle, 96.0, 36.0) : ((source is TextBox) ? CreateSkeleton(source, LuminaSkeletonShape.Rectangle, 220.0, 36.0) : ((source is ComboBox) ? CreateSkeleton(source, LuminaSkeletonShape.Rectangle, 220.0, 36.0) : ((source is ItemsControl) ? BuildListSkeleton(source) : ((source is Border border) ? BuildBorderSkeleton(border) : ((source is Grid grid) ? BuildGridSkeleton(grid) : ((source is StackPanel stackPanel) ? BuildStackPanelSkeleton(stackPanel) : ((source is Panel panel) ? BuildPanelSkeleton(panel) : ((source is LuminaCard card) ? BuildCardSkeleton(card) : ((!(source is ContentControl contentControl)) ? CreateSkeleton(source, LuminaSkeletonShape.Rectangle, 160.0, 32.0) : BuildContentControlSkeleton(contentControl)))))))))))))));
		if (1 == 0)
		{
		}
		return result;
	...
	private Control BuildGridSkeleton(Grid source)
	{
		...
		return (grid.Children.Count > 0) ? ((Control)grid) : ((Control)CreateSkeleton(source, LuminaSkeletonShape.Block, 240.0, 96.0));
	}
	...
	private Control BuildStackPanelSkeleton(StackPanel source)
	{
		...
		return (panel.Children.Count > 0) ? ((Control)panel) : ((Control)CreateSkeleton(source, LuminaSkeletonShape.Block, 240.0, 96.0));
	}
	...
	{
		LuminaCard luminaCard = new LuminaCard();
		luminaCard.IsElevated = source.IsElevated;
		luminaCard.Content = ((source.Content is Control child) ? BuildSkeletonForControl(child) : CreateSkeleton(null, LuminaSkeletonShape.Block, 240.0, 96.0));
		LuminaCard card = luminaCard;
	...
	{
		return (!(source.FontSize >= 16.0) && !source.FontWeight.ToString().Contains("Bold", StringComparison.Ordinal)) ? LuminaSkeletonShape.Text : LuminaSkeletonShape.Title;
	}
	...
	private static LuminaSkeleton CreateSkeleton(Control? source, LuminaSkeletonShape shape, double defaultWidth, double defaultHeight)
	{
		...
		CornerRadius? cornerRadius = ((source == null) ? ((CornerRadius?)null) : Skeleton.GetCornerRadius(source));
		switch (shape)
		{
