...
internal sealed class LuminaTimeWheelPicker : Grid
{
	...
	public LuminaTimeWheelPicker(TimeSpan selectedTime, int minuteIncrement, int secondIncrement, bool useSeconds, bool use12HourClock)
	{
		...
		_hourColumn.ValueChanged += delegate(int value)
		{
			if (!_isUpdating)
			{
		...
		_minuteColumn.ValueChanged += delegate(int value)
		{
			if (!_isUpdating)
			{
				_minute = value;
		...
		{
			_secondColumn.ValueChanged += delegate(int value)
			{
				if (!_isUpdating)
				{
					_second = value;
		...
		{
			_periodColumn.ValueChanged += delegate(int value)
			{
				if (!_isUpdating)
				{
					_isPm = value == 1;
	...
	{
		string format = ((!_use12HourClock) ? (_useSeconds ? "HH:mm:ss" : "HH:mm") : (_useSeconds ? LuminaLocalization.Get("Lumina.Picker.TimeSummaryFormat12HourSeconds") : LuminaLocalization.Get("Lumina.Picker.TimeSummaryFormat12Hour")));
