...
public class LuminaIndexedList : ItemsControl
{
	...
	public static readonly StyledProperty<int> SelectedIndexProperty = AvaloniaProperty.Register<LuminaIndexedList, int>("SelectedIndex", 0, inherits: false, BindingMode.TwoWay);
	...
	public void LoadFlatData<T>(IEnumerable<T> items, Func<T, string> keySelector, Func<T, string> headerSelector, Func<T, Control> itemVisualFactory)
	{
		...
		base.Items.Clear();
		IOrderedEnumerable<IGrouping<string, T>> groups = from g in items.GroupBy(keySelector)
			orderby g.Key
			select g;
		...
		base.ItemsSource = newSections;
		QueueRebuild();
	}

	private List<LuminaIndexedSection> GetSections()
	{
		List<LuminaIndexedSection> result = base.Items.OfType<LuminaIndexedSection>().ToList();
		if (result.Count > 0)
		{
			return result;
		}
		...
		{
			result = base.ItemsSource.OfType<LuminaIndexedSection>().ToList();
			if (result.Count > 0)
			{
				return result;
			}
