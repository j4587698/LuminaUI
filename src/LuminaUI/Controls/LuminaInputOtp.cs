...
public class LuminaInputOtp : TemplatedControl
{
    ...
    public static readonly StyledProperty<string> TextProperty = AvaloniaProperty.Register<LuminaInputOtp, string>(nameof(Text), string.Empty, inherits: false, BindingMode.TwoWay);

    public static readonly StyledProperty<int> LengthProperty = AvaloniaProperty.Register<LuminaInputOtp, int>(nameof(Length), 6);

    public static readonly StyledProperty<bool> IsMaskedProperty = AvaloniaProperty.Register<LuminaInputOtp, bool>(nameof(IsMasked), defaultValue: false);

    public static readonly StyledProperty<double> BoxSizeProperty = AvaloniaProperty.Register<LuminaInputOtp, double>(nameof(BoxSize), 44.0);

    public static readonly StyledProperty<double> BoxFontSizeProperty = AvaloniaProperty.Register<LuminaInputOtp, double>(nameof(BoxFontSize), 18.0);

    public static readonly StyledProperty<bool> AcceptsOnlyDigitsProperty = AvaloniaProperty.Register<LuminaInputOtp, bool>(nameof(AcceptsOnlyDigits), defaultValue: true);

    public static readonly StyledProperty<double> ItemSpacingProperty = AvaloniaProperty.Register<LuminaInputOtp, double>(nameof(ItemSpacing), 8.0);

    public static readonly StyledProperty<ICommand?> CompletedCommandProperty = AvaloniaProperty.Register<LuminaInputOtp, ICommand?>(nameof(CompletedCommand));

    public static readonly DirectProperty<LuminaInputOtp, bool> IsCompleteProperty = AvaloniaProperty.RegisterDirect<LuminaInputOtp, bool>(nameof(IsComplete), (LuminaInputOtp input) => input.IsComplete, null, unsetValue: false);
    ...
    {
        get => GetValue(BoxFontSizeProperty);
    ...
    private void RebuildBoxes()
    {
        ...
        for (int index = 0; index < count; index++)
        {
            ...
            box.Classes.Add("LuminaOtpBox");
            box.Tag = index;
            box.TextChanged += OnBoxTextChanged;
            box.KeyDown += OnBoxKeyDown;
            _boxes.Add(box);
            _itemsHost.Children.Add(box);
        }
        SyncBoxesFromText();
    }
    ...
    private void OnBoxTextChanged(object? sender, TextChangedEventArgs e)
    {
        ...
        box.Text = text;
        Text = string.Concat(_boxes.Select(current => current.Text ?? string.Empty));
    ...
    private void FillFrom(int startIndex, string value)
    {
        ...
        _isSyncing = false;
        Text = string.Concat(_boxes.Select(box => box.Text ?? string.Empty));
    ...
    private string NormalizeText(string? value)
    {
        ...
        IEnumerable<char> filtered = AcceptsOnlyDigits ? value.Where(char.IsDigit) : value.Where(character => !char.IsControl(character));
