...
public class LuminaBreadcrumb : ItemsControl
{
    ...
    static LuminaBreadcrumb()
    {
        ...
        IconBindingProperty = AvaloniaProperty.Register<LuminaBreadcrumb, BindingBase?>(nameof(IconBinding));
        CommandBindingProperty = AvaloniaProperty.Register<LuminaBreadcrumb, BindingBase?>(nameof(CommandBinding));
        CommandParameterBindingProperty = AvaloniaProperty.Register<LuminaBreadcrumb, BindingBase?>(nameof(CommandParameterBinding));
        SeparatorProperty = AvaloniaProperty.Register<LuminaBreadcrumb, object?>(nameof(Separator), "/");
        IconTemplateProperty = AvaloniaProperty.Register<LuminaBreadcrumb, IDataTemplate?>(nameof(IconTemplate));
        ItemsControl.ItemsPanelProperty.OverrideDefaultValue<LuminaBreadcrumb>(DefaultPanel);
        SeparatorProperty.Changed.AddClassHandler((LuminaBreadcrumb breadcrumb, AvaloniaPropertyChangedEventArgs<object?> _) =>
        {
            breadcrumb.RefreshBreadcrumbContainers();
        });
        IconTemplateProperty.Changed.AddClassHandler((LuminaBreadcrumb breadcrumb, AvaloniaPropertyChangedEventArgs<IDataTemplate?> _) =>
        {
            breadcrumb.RefreshBreadcrumbContainers();
        });
    }
    ...
    {
        recycleKey = null;
        return item is not LuminaBreadcrumbItem;
    }
    ...
    protected override void PrepareContainerForItemOverride(Control container, object? item, int index)
    {
        ...
        if (container is not LuminaBreadcrumbItem breadcrumbItem)
        {
            return;
        }
        ...
        if (item is not LuminaBreadcrumbItem)
        {
            ...
            if (DisplayMemberBinding != null && !breadcrumbItem.IsSet(ContentControl.ContentProperty))
            {
                breadcrumbItem.Bind(ContentControl.ContentProperty, DisplayMemberBinding);
            }
    ...
    private void RefreshBreadcrumbContainers()
    {
        ...
        foreach (LuminaBreadcrumbItem item in ItemsPanelRoot.Children.OfType<LuminaBreadcrumbItem>())
        {
    ...
    private void RefreshContainerStates()
    {
        if (ItemsPanelRoot != null)
        {
            LuminaBreadcrumbItem[] items = ItemsPanelRoot.Children.OfType<LuminaBreadcrumbItem>().ToArray();
