...
public class LuminaLinkedCategoryList : ItemsControl
{
    ...
    public static readonly StyledProperty<int> SelectedIndexProperty = AvaloniaProperty.Register<LuminaLinkedCategoryList, int>(nameof(SelectedIndex), 0, inherits: false, BindingMode.TwoWay);

    public static readonly StyledProperty<double> CategoryWidthProperty = AvaloniaProperty.Register<LuminaLinkedCategoryList, double>(nameof(CategoryWidth), 112.0);
    ...
    {
        get => GetValue(CategoryWidthProperty);
    ...
    {
        List<LuminaLinkedCategorySection> result = Items.OfType<LuminaLinkedCategorySection>().ToList();
        if (result.Count > 0)
        {
            return result;
        }
        if (ItemsSource != null)
        {
            result = ItemsSource.OfType<LuminaLinkedCategorySection>().ToList();
            if (result.Count > 0)
            {
                return result;
            }
