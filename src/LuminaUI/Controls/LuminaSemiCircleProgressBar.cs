...
public class LuminaSemiCircleProgressBar : ContentControl
{
    ...
    {
        get => GetValue(AutoTextScaleProperty);
        set => SetValue(AutoTextScaleProperty, value);
    }
    ...
    {
        MinimumProperty = AvaloniaProperty.Register<LuminaSemiCircleProgressBar, double>(nameof(Minimum), 0.0);
        MaximumProperty = AvaloniaProperty.Register<LuminaSemiCircleProgressBar, double>(nameof(Maximum), 100.0);
        ValueProperty = AvaloniaProperty.Register<LuminaSemiCircleProgressBar, double>(nameof(Value), 0.0);
        StrokeThicknessProperty = AvaloniaProperty.Register<LuminaSemiCircleProgressBar, double>(nameof(StrokeThickness), 12.0);
        TrackBrushProperty = AvaloniaProperty.Register<LuminaSemiCircleProgressBar, IBrush?>(nameof(TrackBrush));
        ProgressBrushProperty = AvaloniaProperty.Register<LuminaSemiCircleProgressBar, IBrush?>(nameof(ProgressBrush));
        AutoTextScaleProperty = AvaloniaProperty.Register<LuminaSemiCircleProgressBar, bool>(nameof(AutoTextScale), defaultValue: true);
        DisplayContentProperty = AvaloniaProperty.Register<LuminaSemiCircleProgressBar, object?>(nameof(DisplayContent));
        DisplayTextProperty = AvaloniaProperty.Register<LuminaSemiCircleProgressBar, string>(nameof(DisplayText), "0%");
        HasDefaultContentProperty = AvaloniaProperty.Register<LuminaSemiCircleProgressBar, bool>(nameof(HasDefaultContent), defaultValue: true);
        HasCustomContentProperty = AvaloniaProperty.Register<LuminaSemiCircleProgressBar, bool>(nameof(HasCustomContent), defaultValue: false);
        DisplayFontSizeProperty = AvaloniaProperty.Register<LuminaSemiCircleProgressBar, double>(nameof(DisplayFontSize), 32.0);
        DisplayContentMarginProperty = AvaloniaProperty.Register<LuminaSemiCircleProgressBar, Thickness>(nameof(DisplayContentMargin));
    ...
    private void UpdateDisplayContent()
    {
        ...
        DisplayContent = hasCustomContent ? Content : percentText;
    ...
    private void UpdateDisplayMetrics()
    {
        ...
        if (metrics.Radius <= 0.0)
        {
            ...
            DisplayContentMargin = default;
            return;
        }
        DisplayFontSize = AutoTextScale ? Math.Clamp(metrics.Radius * 0.46, 20.0, 72.0) : FontSize;
        double visualCenterY = metrics.Center.Y - metrics.Radius * 0.24;
        double verticalOffset = visualCenterY - Bounds.Height / 2.0;
    ...
    {
        double thickness = Math.Max(1.0, StrokeThickness);
        double radius = Math.Min(Math.Max(0.0, Bounds.Width - thickness) / 2.0, Math.Max(0.0, Bounds.Height - thickness));
        return new ArcMetrics(new Point(Bounds.Width / 2.0, thickness / 2.0 + radius), radius);
    }
