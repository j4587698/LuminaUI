...
public class LuminaLoadingVisual : Control
{
    ...
    {
        IsActiveProperty = AvaloniaProperty.Register<LuminaLoadingVisual, bool>(nameof(IsActive), defaultValue: true);
        KindProperty = AvaloniaProperty.Register<LuminaLoadingVisual, LuminaLoadingKind>(nameof(Kind), LuminaLoadingKind.Ring);
        SizeProperty = AvaloniaProperty.Register<LuminaLoadingVisual, double>(nameof(Size), 22.0);
        StrokeThicknessProperty = AvaloniaProperty.Register<LuminaLoadingVisual, double>(nameof(StrokeThickness), 3.0);
        BrushProperty = AvaloniaProperty.Register<LuminaLoadingVisual, IBrush?>(nameof(Brush));
    ...
    private void RenderRing(DrawingContext context, IBrush brush)
    {
        ...
        Point center = new Point(Bounds.Width / 2.0, Bounds.Height / 2.0);
    ...
    private void RenderBar(DrawingContext context, IBrush brush)
    {
        ...
        double height = Math.Max(8.0, Bounds.Height);
    ...
    private void RenderDots(DrawingContext context, IBrush brush)
    {
        ...
        double baseRadius = Math.Max(2.2, Math.Min(Bounds.Height, spacing) * 0.22);
        for (int i = 0; i < 4; i++)
        {
