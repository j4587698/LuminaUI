...
public class LuminaNavigationView : ItemsControl
{
	...
	public static readonly StyledProperty<object?> SelectedItemProperty = AvaloniaProperty.Register<LuminaNavigationView, object?>("SelectedItem", null, inherits: false, BindingMode.TwoWay);

	public static readonly StyledProperty<string?> SelectedKeyProperty = AvaloniaProperty.Register<LuminaNavigationView, string?>("SelectedKey", null, inherits: false, BindingMode.TwoWay);
	...
	public string? SelectedKey
	{
		get
		{
			return GetValue(SelectedKeyProperty);
		}
		set
		{
	...
	private void SelectItem(LuminaNavigationItem? item, bool updateKey, bool raiseSelectionChanged, RoutedEventArgs? eventArgs = null)
	{
		...
		{
			this.SelectionChanged?.Invoke(this, eventArgs ?? new RoutedEventArgs());
		}
	}
	...
	private static IEnumerable<LuminaNavigationItem> EnumerateNavigationItems(ItemsControl owner)
	{
		...
		{
			yield return item;
			foreach (LuminaNavigationItem item2 in EnumerateNavigationItems(item))
			{
				yield return item2;
			}
		}
	...
	private static IEnumerable<LuminaNavigationItem> EnumerateDirectNavigationItems(ItemsControl owner)
	{
		...
		foreach (object? item2 in EnumerateItems(owner.ItemsSource))
		{
			if (item2 is LuminaNavigationItem navigationItem2)
			{
				yield return navigationItem2;
			}
		}
	}
