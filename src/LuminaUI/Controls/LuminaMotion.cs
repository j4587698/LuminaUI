...
public sealed class LuminaMotion
{
	...
	static LuminaMotion()
	{
		...
		AnimateLayoutProperty.Changed.AddClassHandler(delegate(Control control, AvaloniaPropertyChangedEventArgs _)
		{
			ApplyOrClear(control);
		});
		AnimateSizeProperty.Changed.AddClassHandler(delegate(Control control, AvaloniaPropertyChangedEventArgs _)
		{
			ApplyOrClear(control);
		});
		AnimateOpacityProperty.Changed.AddClassHandler(delegate(Control control, AvaloniaPropertyChangedEventArgs _)
		{
			ApplyOrClear(control);
		});
		DurationProperty.Changed.AddClassHandler(delegate(Control control, AvaloniaPropertyChangedEventArgs _)
		{
			ApplyOrClear(control);
		});
		EasingProperty.Changed.AddClassHandler(delegate(Control control, AvaloniaPropertyChangedEventArgs _)
		{
			ApplyOrClear(control);
		});
		TranslateXProperty.Changed.AddClassHandler(delegate(Control control, AvaloniaPropertyChangedEventArgs _)
		{
			ApplyTransformTarget(control);
		});
		TranslateYProperty.Changed.AddClassHandler(delegate(Control control, AvaloniaPropertyChangedEventArgs _)
		{
			ApplyTransformTarget(control);
		});
		ScaleProperty.Changed.AddClassHandler(delegate(Control control, AvaloniaPropertyChangedEventArgs _)
		{
			ApplyTransformTarget(control);
		});
		RotationProperty.Changed.AddClassHandler(delegate(Control control, AvaloniaPropertyChangedEventArgs _)
		{
			ApplyTransformTarget(control);
		});
		HoverScaleProperty.Changed.AddClassHandler(delegate(Control control, AvaloniaPropertyChangedEventArgs _)
		{
			ApplyPointerHooks(control);
		});
		PressedScaleProperty.Changed.AddClassHandler(delegate(Control control, AvaloniaPropertyChangedEventArgs _)
		{
			ApplyPointerHooks(control);
		});
		FadeInProperty.Changed.AddClassHandler(delegate(Control control, AvaloniaPropertyChangedEventArgs _)
		{
			ApplyFadeIn(control);
		});
		FadeInScaleProperty.Changed.AddClassHandler(delegate(Control control, AvaloniaPropertyChangedEventArgs _)
		{
			ApplyFadeIn(control);
		});
		VisibleProperty.Changed.AddClassHandler(delegate(Control control, AvaloniaPropertyChangedEventArgs _)
		{
			ApplyVisibility(control);
		});
		HiddenScaleProperty.Changed.AddClassHandler(delegate(Control control, AvaloniaPropertyChangedEventArgs _)
		{
			ApplyVisibility(control);
		});
		DisabledOpacityProperty.Changed.AddClassHandler(delegate(Control control, AvaloniaPropertyChangedEventArgs _)
		{
			ApplyDisabledState(control);
		});
		DisabledScaleProperty.Changed.AddClassHandler(delegate(Control control, AvaloniaPropertyChangedEventArgs _)
		{
			ApplyDisabledState(control);
		});
		InputElement.IsEnabledProperty.Changed.AddClassHandler(delegate(Control control, AvaloniaPropertyChangedEventArgs _)
		{
			ApplyDisabledState(control);
		});
		SquishProperty.Changed.AddClassHandler(delegate(Control control, AvaloniaPropertyChangedEventArgs _)
		{
			ApplySquishHooks(control);
		});
		SquishIntensityProperty.Changed.AddClassHandler(delegate(Control control, AvaloniaPropertyChangedEventArgs _)
		{
			ApplySquishHooks(control);
		});
		SquishDepthProperty.Changed.AddClassHandler(delegate(Control control, AvaloniaPropertyChangedEventArgs _)
		{
			ApplySquishHooks(control);
		});
		SquishTiltProperty.Changed.AddClassHandler(delegate(Control control, AvaloniaPropertyChangedEventArgs _)
		{
			ApplySquishHooks(control);
		});
	}
	...
	internal static async Task AnimateVisualTransformAsync(Control control, double x, double y, double scaleX, double scaleY, TimeSpan? duration = null, Easing? easing = null, CancellationToken cancellationToken = default(CancellationToken))
	{
		ArgumentNullException.ThrowIfNull(control, "control");
		MotionState state = GetOrCreateState(control);
	...
	internal static async Task AnimateVisualTransformAsync(Control control, double x, double y, double scaleX, double scaleY, double rotation, TimeSpan? duration = null, Easing? easing = null, CancellationToken cancellationToken = default(CancellationToken))
	{
		ArgumentNullException.ThrowIfNull(control, "control");
		TopLevel? topLevel = default(TopLevel);
		int num;
		if (control.IsAttachedToVisualTree())
		{
			topLevel = TopLevel.GetTopLevel(control);
			num = ((topLevel == null) ? 1 : 0);
		}
		else
		{
			num = 1;
		...
		TimeSpan animationDuration = duration ?? TimeSpan.FromMilliseconds(280L);
