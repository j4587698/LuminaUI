...
public class LuminaResponsivePanel : Panel
{
	...
	public static readonly StyledProperty<double> MinItemWidthProperty = AvaloniaProperty.Register<LuminaResponsivePanel, double>("MinItemWidth", 220.0);

	public static readonly StyledProperty<double> MaxItemWidthProperty = AvaloniaProperty.Register<LuminaResponsivePanel, double>("MaxItemWidth", double.PositiveInfinity);

	public static readonly StyledProperty<double> HorizontalSpacingProperty = AvaloniaProperty.Register<LuminaResponsivePanel, double>("HorizontalSpacing", 12.0);

	public static readonly StyledProperty<double> VerticalSpacingProperty = AvaloniaProperty.Register<LuminaResponsivePanel, double>("VerticalSpacing", 12.0);

	public static readonly StyledProperty<int> MaxColumnsProperty = AvaloniaProperty.Register<LuminaResponsivePanel, int>("MaxColumns", int.MaxValue);

	public double MinItemWidth
	{
		get
		{
			return GetValue(MinItemWidthProperty);
		}
		set
		{
	...
	public double MaxItemWidth
	{
		get
		{
			return GetValue(MaxItemWidthProperty);
		}
		set
		{
	...
	public double HorizontalSpacing
	{
		get
		{
			return GetValue(HorizontalSpacingProperty);
		}
		set
		{
	...
	public double VerticalSpacing
	{
		get
		{
			return GetValue(VerticalSpacingProperty);
		}
		set
		{
	...
	public int MaxColumns
	{
		get
		{
			return GetValue(MaxColumnsProperty);
		}
		set
		{
	...
	{
		Control[] visibleChildren = base.Children.Where((Control control) => control.IsVisible).ToArray();
	...
	protected override Size ArrangeOverride(Size finalSize)
	{
		...
		{
			int childrenInRow = Math.Min(layout.Columns, visibleChildren.Length - row2 * layout.Columns);
			double x = 0.0;
			for (int column = 0; column < childrenInRow; column++)
			{
				Control child = visibleChildren[row2 * layout.Columns + column];
				child.Arrange(new Rect(x, y, layout.ItemWidth, rowHeights[row2]));
				x += layout.ItemWidth + layout.HorizontalSpacing;
			}
			y += rowHeights[row2] + layout.VerticalSpacing;
		}
		return finalSize;
	}
